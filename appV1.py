...
------------------------------------------------------
Logic is unchanged from the original. This revision focuses on:
  • A polished visual theme (custom CSS, gradient header, card layouts)
  • Better data viz (Plotly gauge + horizontal bar chart with color bands)
  • Color-coded subject cards (red / amber / green) for quick scanning
  • A timeline-style daily planner
  • A nicer login screen and quote card
"""
...
# ---------------------------------------------------------------
# 1. RULE SETS  (unchanged — the "AI" expert system)
# ---------------------------------------------------------------
SUGGESTIONS_RULES = {
    "Excellent": [
...
# Visual palette tied to performance bands
BAND_COLORS = {
...
def build_planner(performance, weak_subjects, all_subjects):
    ...
    for time_slot, activity in template:
        ...
        if "weak subject" in activity.lower() and focus:
            tag = focus[weak_index % len(focus)]
            weak_index += 1
        elif "subject rotation" in activity.lower() and all_subjects:
            tag = all_subjects[rotate_index % len(all_subjects)]
            rotate_index += 1
        planner.append((time_slot, text, tag))
    return planner
...
.tl-time {
    min-width: 145px; font-weight: 700; color: #4f46e5; font-size: 13px;
}
...
.quote-card {
    ...
    font-style: italic; font-size: 18px; color: #78350f;
    line-height: 1.6;
}
...
if not st.session_state.logged_in:
    ...
    with col2:
        ...
        <div class="login-card">
            ...
            <h1 style="margin:8px 0 4px 0; color:#1e293b;">AI Study Assistant</h1>
...
with tab_home:
    ...
    for col, (icon, title, desc) in zip([c1, c2, c3], cards):
        ...
        <div class="card" style="text-align:center; min-height:140px;">
            ...
            <div style="font-weight:700; font-size:17px; margin-top:6px; color:#1e293b;">{title}</div>
    ...
    st.markdown("")
    st.info("💡 **How it works:** A rule-based expert system — if-elif logic plus curated knowledge "
            "dictionaries — decides performance bands, study tips, and your daily schedule.")


# ===== ENTER MARKS =====
with tab_marks:
    st.markdown("### ✏️ Subject-wise Marks Entry")
    st.caption("Enter marks out of 100 for each subject. Use the Save button at the bottom.")
    ...
    with st.form("marks_form"):
        ...
        for i in range(int(num)):
            with cols[i % 2]:
                ...
                mark = st.slider(f"Marks for {sub_default}", 0, 100, 75, key=f"mark_{i}")
                if sub.strip():
                    new_marks[sub.strip()] = mark
        submitted = st.form_submit_button("💾 Save Marks", type="primary", use_container_width=True)
        if submitted:
    ...
    if not st.session_state.marks:
        ...
        for col, (label, val, emoji) in zip(
            ...
            <div class="card" style="text-align:center;">
                ...
                <div style="color:#64748b; font-size:12px; text-transform:uppercase; margin-top:4px;">{label}</div>
                <div style="font-weight:700; font-size:20px; color:#0f172a; margin-top:4px;">{val}</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        with right:
            ...
            bar.update_layout(
                height=280, margin=dict(l=10, r=30, t=20, b=20),
                xaxis=dict(range=[0, 110], showgrid=True, gridcolor='#eef2f7'),
        ...
        for sub, m in sorted(marks.items(), key=lambda x: -x[1]):
            ...
            <div class="subject-row" style="border-left-color:{c['bg']};">
                ...
                <span class="subject-mark" style="color:{c['bg']};">{m} / 100</span>
            </div>
            """, unsafe_allow_html=True)
...
with tab_suggest:
    ...
    if not st.session_state.marks:
        ...
        <div class="card" style="background:{band['soft']}; border-left:4px solid {band['bg']};">
            <div style="font-size:13px; color:{band['text']}; text-transform:uppercase; letter-spacing:0.5px;">
                Detected Band
            </div>
            <div style="font-size:22px; font-weight:700; color:{band['text']}; margin-top:4px;">
                {band['emoji']} {perf} &nbsp; · &nbsp; {pct}%
            </div>
        </div>
        """, unsafe_allow_html=True)
...
with tab_plan:
    ...
    if not st.session_state.marks:
        ...
        st.markdown(f"""
        <div class="card" style="background:{band['soft']}; border-left:4px solid {band['bg']};">
            <b style="color:{band['text']};">{band['emoji']} {perf} band</b>
            &nbsp;·&nbsp; Weak subjects: <b>{', '.join(weak) if weak else 'None 🎉'}</b>
        </div>
        """, unsafe_allow_html=True)

        plan = build_planner(perf, weak, list(marks.keys()))
        for time_slot, activity, tag in plan:
